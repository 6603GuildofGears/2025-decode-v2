...
public class Blue_Back_Motif extends OpMode {
    ...
    private double shootRpm = 4100;     // High RPM for back position
    private double shootHood = 0.5;     // Hood angle
    private static final int GOAL_TAG_ID = 20; // blue goal

    // Turret auto-aim - BLUE BACK ANGLES
    private static final double TURRET_TARGET_DEG = 120;  // Shooting angle for back position
    private static final double TURRET_MOTIF_SCAN_DEG = 140;  // Angle to scan for motif tag
    ...
    private boolean scanningForMotif = false;  // True while turret scans at 150° for motif
    private boolean turretAtShootAngle = false;  // True when turret reaches 120° after scan
    ...
    // Poses (same as Blue_Back original)
    ...
    private void updateTurret() {
        // Use motif scan angle when scanning, otherwise use shoot angle
        ...
        switch (pathState) {
            case DRIVE_STARTPOSE_TO_SHOOTPOSE:
                if (!pathStarted) {
                    ...
                    // Start motif scanning (turret will aim at 150°)
                ...
                // Scan for motif tag while turret is at 150°
                if (scanningForMotif) {
                    if (!motifLocked) {
            ...
            case DRIVE_INTAKEPOSE_TO_INTAKE1:
                if (!pathStarted) {
                    follower.followPath(driveIntakePoseToIntake1, 0.625, true);
                    pathStarted = true;
            ...
            case DRIVE_INTAKE2_TO_SHOOTPOSE:
                if (!pathStarted) {
                    follower.followPath(driveIntake2ToShootPose, 0.625, true);
                    pathStarted = true;
    ...
    public void loop() {
        ...
        // RPM Diagnostics
        double actualVelocity = flywheel.getVelocity();  // ticks per second
        double actualRpm = actualVelocity * 60.0 / 28.0;  // convert to RPM
        telemetry.addData("Shooter RPM", String.format("%.0f / %.0f", actualRpm, shootRpm));
        telemetry.addData("RPM Error", String.format("%.0f", actualRpm - shootRpm));
        
        // Auto-Reverse Features Status
        telemetry.addData("Unjam", unjamming ? "ON" : "OFF");
        telemetry.addData("Full Reverse", fullSpindexerReversing ? "ON" : "OFF");
