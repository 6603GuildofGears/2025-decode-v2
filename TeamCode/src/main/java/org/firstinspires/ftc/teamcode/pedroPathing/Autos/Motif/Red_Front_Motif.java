 ...
 * MIRRORED FROM BLUE FRONT with bridge pose for turret transition.
 */
...
public class Red_Front_Motif extends OpMode {
    ...
    // Shoot pose (95, 84) to red basket - approximately 30-35 inches
    private double shootRpm = 3250;     // From ShooterLookup for ~30-35"
    private double shootHood = 0.5;     // From ShooterLookup for ~30-35"
    private static final int GOAL_TAG_ID = 10; // red goal

    // End position shooter settings
    // End pose (89, 72) to red basket - different angle/distance
    ...
    // Turret auto-aim
    private static final double TURRET_TARGET_DEG = 5;  // Shooting angle for initial shootPose
    private static final double TURRET_END_SHOOT_DEG = 7;  // Shooting angle for endPose  
    private static final double TURRET_MOTIF_SCAN_DEG = 0;  // Angle to scan for motif tag (same as shoot for red)
    ...
    private boolean scanningForMotif = false;  // True while turret scans at 0° for motif
    private boolean turretAtShootAngle = false;  // True when turret reaches 0° after scan
    ...
    // Poses (mirrored from Blue_Front across x=72, Y stays same, angles mirrored across 90°)
    private final Pose startPose = new Pose(124, 120, Math.toRadians(36));
    private final Pose shootPose = new Pose(95, 84, Math.toRadians(0));
    private final Pose intake1 = new Pose(107, 76, Math.toRadians(0));       // Intake 1 position
    private final Pose intake1Pose2 = new Pose(108.5, 76, Math.toRadians(0));  // Intake 1 waypoint 2
    private final Pose intake1Pose3 = new Pose(116, 76, Math.toRadians(0));  // Intake 1 waypoint 3
    private final Pose intake1Pose4 = new Pose(123, 76, Math.toRadians(0));  // Intake 1 waypoint 4 (2 inches forward)
    private final Pose intake2 = new Pose(99, 56, Math.toRadians(0));       // Intake 2 position
    private final Pose intake2Pose1 = new Pose(108.5, 56, Math.toRadians(0));  // Intake 2 waypoint 1
    private final Pose intake2Pose2 = new Pose(116, 56, Math.toRadians(0));  // Intake 2 waypoint 2
    private final Pose intake2Pose3 = new Pose(123, 56, Math.toRadians(0));  // Intake 2 waypoint 3
    private final Pose endPose = new Pose(89,90, Math.toRadians(0));       // Final shoot position
        ...
        switch (pathState) {
            case DRIVE_STARTPOSE_TO_SHOOTPOSE:
                if (!pathStarted) {
                    ...
                    // Start motif scanning (turret will aim at 0°)
                ...
                // Scan for motif tag while turret is at 0°
                if (scanningForMotif) {
                    if (!motifLocked) {
    ...
    @Override
    public void init() {
        pathState = PathState.DRIVE_STARTPOSE_TO_SHOOTPOSE;  // Start by driving to shoot
        pathTimer = new Timer();
    ...
    public void start() {
        ...
        // Note: fallback is now handled during DRIVE_STARTPOSE_TO_BRIDGEPOSE if motif not detected
    }
