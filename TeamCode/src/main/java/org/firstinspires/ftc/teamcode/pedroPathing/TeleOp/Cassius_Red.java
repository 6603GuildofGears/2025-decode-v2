...
public class Cassius_Red extends LinearOpMode {
    ...
    double SMOOTHING_ALPHA = 0.3; // 0.0 = very smooth/slow, 1.0 = no smoothing (raw)
        ...
        while (opModeIsActive()) {
            ...
            //   Motif fire state 5 also triggers a shoot
            ...
            if (motifState == 3) {
                ...
                try {
                    ...
                    if (redGoal != null) {
                        ...
                        // No red goal visible — freeze turret in place
                        turretPower = 0;
                        turretIntegral = 0;
                        turretLastError = 0;
                        turretMode = "HOLDING";
            ...
            // Motif fire: state 4 — once target is seen, shoot immediately
            if (motifState == 4) {
            ...
            telemetry.addData("Motif Enabled", motifEnabled ? "YES [X2 toggle]" : "NO [X2 toggle]");
            String[] motifLabels = {"IDLE", "SCAN_TAG", "SCAN_BALLS", "TURN→20°", "LOCKING", "SHOOTING"};
            telemetry.addData("Motif Fire", motifLabels[motifState]);
            if (detectedMotifTag != -1) {
